...
import { createAppointment, findAgent, findAppointment, getDashboard, listAgents, listAilments, listTherapies, openDatabase } from "../src/db/index.js";
...
describe("clinic database", () => {
  it("migrates and seeds the exact reference data idempotently", () => {
    ...
    migrateDatabase(db);
    seedDatabase(db);
    expect((db.prepare("SELECT COUNT(*) AS count FROM schema_migrations").get() as { count: number }).count).toBe(6);
