...
export function renderNotification(notification: NotificationDelivery): NotificationMessage {
  ...
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>${escapeHtml(copy.subject)}</title></head><body><main><h1>${escapeHtml(copy.heading)}</h1><p>${escapeHtml(copy.detail)}</p><dl><dt>Agent</dt><dd>${escapeHtml(notification.agent_name)}</dd><dt>Therapist</dt><dd>${escapeHtml(notification.therapist_name)}</dd><dt>When</dt><dd>${escapeHtml(formatAppointment(notification.appointment_scheduled_at))}</dd></dl><p>Contact the clinic if you need help.</p></main></body></html>`;
